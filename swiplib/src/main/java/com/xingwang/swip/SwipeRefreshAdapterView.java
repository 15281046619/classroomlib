...
public abstract class SwipeRefreshAdapterView<T extends View> extends BaseSwipeRefresh<T>
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        int[] crLayout = R.styleable.swip_crLayout;
        if (crLayout != null && crLayout.length > 0) {
            final TypedArray a = context.obtainStyledAttributes(attrs, crLayout, defStyleAttr,
                    defStyleRes);
            if (a != null) {
                mLoadResource = a.getResourceId(R.styleable.swip_crLayout_load_layout, 0);
                mLoadText = a.getString(R.styleable.swip_crLayout_load_text);
                mLoadIndeterminateDrawable = a.getDrawable(R.styleable
                        .swip_crLayout_load_indeterminate_drawable);
                mCompletedText = a.getString(R.styleable.swip_crLayout_load_completed_text);
                isLoadAnimator = a.getBoolean(R.styleable.swip_crLayout_load_animator, false);
                a.recycle();
            }
        }
    }
    ...
    private void initShowLoadAnimator() {
        ...
        mShowLoadAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mLoadLayout.getLayoutParams();
    ...
    private void addLoadLayout() {
        if (mLoadLayout == null) {
            ...
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(mLoadLayout.getLayoutParams());
