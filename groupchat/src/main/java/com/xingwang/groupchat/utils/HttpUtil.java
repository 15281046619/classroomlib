...
public class HttpUtil {
     ...
     * @param tag      网络请求tag
     */
    public static void get(String url, HttpCallBack callBack, String tag) {
        commonGet(getRequestForGet(url, null, tag), callBack,tag);
    }
     ...
     * @param tag      网络请求tag
     */
    public static void get(String url, HashMap<String, String> params, HttpCallBack callBack, String tag) {
        commonGet(getRequestForGet(url, params, tag), callBack,tag);
    }
     ...
     * @param tag      网络请求tag
     */
    public static void post(String url, HashMap<String, String> params, HttpCallBack callBack, String tag) {
        commonPost(getRequestForGet(url, params, tag), callBack,tag);
    }
    ...
    private static void commonGet(Request request,final HttpCallBack callBack) {
        commonGet(request,callBack,null);
    }

    /**
     * GET请求 公共请求部分
     */
    private static void commonGet(Request request,final HttpCallBack callBack,String tag) {
    ...
    private static <T extends Serializable> void commonPost(Request request,final HttpCallBack callBack) {
        commonPost(request,callBack,null);
    }

    /**
     * POST请求 公共请求部分
     */
    private static <T extends Serializable> void commonPost(Request request, final HttpCallBack callBack,String tag) {
        if (request == null) return;
        Call call = getInstance().newCall(request);
        call.enqueue(new Callback() {
            @Override
            public void onFailure(@NonNull Call call, @NonNull final IOException e) {
                mainHandler.post(() -> {
                    Log.i("POST","onFailure:"+e.getMessage());
    ...
    private static void handleResult(CommonEntity commonEntity,HttpCallBack callBack,String tag){
        if (EmptyUtils.isEmpty(commonEntity)){
            mainHandler.post(() ->  callBack.onFailure("数据解析错误!"));
            return;
        }
        if (commonEntity.getStatus()==1){
            mainHandler.post(() -> callBack.onSuccess(JsonUtils.objectToJson(commonEntity.getData()),tag));
        }else {
    ...
    public interface HttpCallBack {
        ...
        void onSuccess(String json,String tag);
    }
}
