...
public class CircleImageView extends AppCompatImageView {
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.circle_imgView);
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.circle_imgView_android_src) {
            mDefaultImageId = typedArray.getResourceId(attr, 0);
        } else if (attr == R.styleable.circle_imgView_iv_circle) {
            mCircle = typedArray.getBoolean(attr, mCircle);
        } else if (attr == R.styleable.circle_imgView_iv_cornerRadius) {
            mCornerRadius = typedArray.getDimensionPixelSize(attr, mCornerRadius);
        } else if (attr == R.styleable.circle_imgView_iv_square) {
            mSquare = typedArray.getBoolean(attr, mSquare);
        } else if (attr == R.styleable.circle_imgView_iv_borderWidth) {
            mBorderWidth = typedArray.getDimensionPixelSize(attr, mBorderWidth);
        } else if (attr == R.styleable.circle_imgView_iv_borderColor) {
