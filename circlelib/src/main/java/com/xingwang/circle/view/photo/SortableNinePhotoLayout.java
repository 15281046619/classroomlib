...
import com.xingwang.circle.view.photo.listener.OnItemChildClickListener;
 ...
 * 描述:拖拽排序九宫格图片控件
 */
public class SortableNinePhotoLayout extends RecyclerView implements OnItemChildClickListener, OnRVItemClickListener {
    ...
    private void initDefaultAttrs() {
        ...
        mSortable = true;
        mEditable = true;
        mDeleteDrawableResId = R.mipmap.circle_pp_ic_delete;
        mDeleteDrawableOverlapQuarter = false;
        mMaxItemCount = 9;
        mItemSpanCount = 3;
        mItemWidth = 0;
        mItemCornerRadius = 0;
        mPlusDrawableResId = R.mipmap.circle_add_pic;
        mItemWhiteSpacing = ConvertUtils.dp2px(4);
        mPlaceholderDrawableResId = R.mipmap.circle_pp_ic_holder_light;
        mOtherWhiteSpacing = ConvertUtils.dp2px(100);
    }

    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.circle_sortableNinePhotoLayout);
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_plusEnable) {
            mPlusEnable = typedArray.getBoolean(attr, mPlusEnable);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_sortable) {
            mSortable = typedArray.getBoolean(attr, mSortable);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_deleteDrawable) {
            mDeleteDrawableResId = typedArray.getResourceId(attr, mDeleteDrawableResId);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_deleteDrawableOverlapQuarter) {
            mDeleteDrawableOverlapQuarter = typedArray.getBoolean(attr, mDeleteDrawableOverlapQuarter);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_maxItemCount) {
            mMaxItemCount = typedArray.getInteger(attr, mMaxItemCount);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_itemSpanCount) {
            mItemSpanCount = typedArray.getInteger(attr, mItemSpanCount);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_plusDrawable) {
            mPlusDrawableResId = typedArray.getResourceId(attr, mPlusDrawableResId);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_itemCornerRadius) {
            mItemCornerRadius = typedArray.getDimensionPixelSize(attr, 0);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_itemWhiteSpacing) {
            mItemWhiteSpacing = typedArray.getDimensionPixelSize(attr, mItemWhiteSpacing);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_otherWhiteSpacing) {
            mOtherWhiteSpacing = typedArray.getDimensionPixelOffset(attr, mOtherWhiteSpacing);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_placeholderDrawable) {
            mPlaceholderDrawableResId = typedArray.getResourceId(attr, mPlaceholderDrawableResId);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_editable) {
            mEditable = typedArray.getBoolean(attr, mEditable);
        } else if (attr == R.styleable.circle_sortableNinePhotoLayout_bga_snpl_itemWidth) {
    ...
    private void afterInitDefaultAndCustomAttrs() {
        if (mItemWidth == 0) {
            mItemWidth = (ScreenUtils.getScreenWidth() - mOtherWhiteSpacing) / mItemSpanCount;
        } else {
        ...
        addItemDecoration(GridDivider.newInstanceWithSpacePx(mItemWhiteSpacing / 2));
    ...
    private void calculatePhotoTopRightMargin() {
        if (mDeleteDrawableOverlapQuarter) {
            ...
            int deleteDrawablePadding = getResources().getDimensionPixelOffset(R.dimen.circle_delete_padding);
    ...
    private class PhotoAdapter extends RecyclerViewAdapter<String> {
        ...
        public PhotoAdapter(RecyclerView recyclerView) {
            super(recyclerView, R.layout.circle_pp_item_nine_photo);
            mImageSize = ScreenUtils.getScreenWidth() / (mItemSpanCount > 3 ? 8 : 6);
        }
        ...
        protected void fillData(ViewHolderHelper helper, int position, String model) {
            ViewGroup.MarginLayoutParams params = (ViewGroup.MarginLayoutParams) helper.getView(R.id.iv_item_nine_photo).getLayoutParams();
            params.setMargins(0, mPhotoTopRightMargin, mPhotoTopRightMargin, 0);

            NiceImageView imageView = helper.getView(R.id.iv_item_nine_photo);
            ...
            if (isPlusItem(position)) {
                ...
                helper.setImageResource(R.id.iv_item_nine_photo, mPlusDrawableResId);
            } else {
                if (mEditable) {
                ...
                GlideUtils.loadPic(model, helper.getImageView(R.id.iv_item_nine_photo), getContext().getApplicationContext(), mImageSize);
            }
        }
    }
    ...
    private class ItemTouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder source, RecyclerView.ViewHolder target) {
            ...
            if (mDelegate != null) {
                mDelegate.onNinePhotoItemExchanged(SortableNinePhotoLayout.this, source.getAdapterPosition(), target.getAdapterPosition(), getData());
            }
            return true;
        }
        ...
        public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
            if (actionState != ACTION_STATE_IDLE) {
                ...
                ((RecyclerViewHolder) viewHolder).getViewHolderHelper().getImageView(R.id.iv_item_nine_photo).setColorFilter(getResources().getColor(R.color.circle_pp_photo_selected_mask));
            }
        ...
        public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
            ...
            ((RecyclerViewHolder) viewHolder).getViewHolderHelper().getImageView(R.id.iv_item_nine_photo).setColorFilter(null);
            super.clearView(recyclerView, viewHolder);
        }
    }

    public interface Delegate {
        void onClickAddNinePhotoItem(SortableNinePhotoLayout sortableNinePhotoLayout, View view, int position, ArrayList<String> models);

        void onClickDeleteNinePhotoItem(SortableNinePhotoLayout sortableNinePhotoLayout, View view, int position, String model, ArrayList<String> models);

        void onClickNinePhotoItem(SortableNinePhotoLayout sortableNinePhotoLayout, View view, int position, String model, ArrayList<String> models);

        void onNinePhotoItemExchanged(SortableNinePhotoLayout sortableNinePhotoLayout, int fromPosition, int toPosition, ArrayList<String> models);
    }
}